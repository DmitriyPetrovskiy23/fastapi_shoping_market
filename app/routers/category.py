...
async def create_category(db: Annotated[AsyncSession, Depends(get_db)], create_category: CreateCategory, get_user: Annotated[dict,
...
async def delete_category(db: Annotated[AsyncSession, Depends(get_db)], category_id: int, get_user: Annotated[dict, Depends(get_current_user)]):
    if get_user.get('is_admin'):
        ...
        """Если хотим удалаять, то разкомментировать"""
        # db.delete(category)
        await db.commit()
        return {
            'status_code': status.HTTP_200_OK,
            'transaction': 'Category delete is succesfull'
        }
    else:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
